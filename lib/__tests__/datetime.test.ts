...
describe("时间工具", () => {
  it("格式化为后端约定的格式", () => {
    ...
    expect(formatDateTime(date)).toBe("2025-01-02 03:04:05")
    expect(formatDateTime(date, false)).toBe("2025-01-02 03:04")
  })

  it("兼容 '.' 与 '-' 两种分隔符", () => {
    expect(parseDateTime("2025.01.02 03:04")?.getFullYear()).toBe(2025)
    expect(parseDateTime("2025-01-02 03:04")?.getMonth()).toBe(0)
    expect(parseDateTime("")).toBeUndefined()
    expect(parseDateTime("不是时间")).toBeUndefined()
  })

  it("截取日期部分", () => {
    expect(toDateString("2025-01-02 03:04:05")).toBe("2025-01-02")
